...
java -d64 -XX:MaxHeapSize=5000m -jar ../splitter-r439/splitter.jar --max-nodes=%max_split_nodes% --max-areas=512 --mapid=%mapid%  --keep-complete=false --description="%description%" --mixed %srtm_base%%srtm_file% ../download/%download_name%
if %errorlevel% neq 0 exit /b %errorlevel%
